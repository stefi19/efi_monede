...
import {
  Eye, EyeOff, Bell, Send, Plus, ArrowLeftRight,
  Zap, ChevronRight, TrendingUp, TrendingDown,
} from 'lucide-react';
...
export default function Home() {
  ...
  if (!user) return (
    <div className="flex items-center justify-center bg-[#0a0a0a]" style={{ height: '100%' }}>
  ...
  const totalIn   = user.transactions
    .filter((t) => t.type === 'receive' || t.type === 'topup')
    .reduce((s, t) => s + t.amount, 0);
  const totalOut  = user.transactions
    ...
    <div className="bg-[#0a0a0a] pb-safe-nav">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-5">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full bg-gradient-to-br ${userGradients[user.id] ?? 'from-[#7c6af7] to-[#a78bfa]'} flex items-center justify-center text-sm font-bold text-white shrink-0`}
          >
            {user.avatar}
          </div>
        ...
        <button
          ...
          className="relative w-10 h-10 rounded-full glass flex items-center justify-center shrink-0 active:scale-95 transition-transform"
        >
          <Bell size={18} className={notifPerm === 'granted' ? 'text-[#7c6af7]' : 'text-gray-400'} />
          <span
            className={`absolute top-2 right-2 w-2 h-2 rounded-full ${notifPerm === 'granted' ? 'bg-[#7c6af7]' : 'bg-red-500'}`}
          />
        </button>
      </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                ...
                className="text-gray-400 p-1 -mr-1 active:scale-95 transition-transform"
              >
                {balanceVisible ? <Eye size={15} /> : <EyeOff size={15} />}
              </button>
            </div>
            <div className="flex items-baseline gap-2 mb-3">
              <span className="text-[34px] leading-none font-bold text-white">
                {balanceVisible ? `Ɛ${user.balance.toLocaleString()}` : '••••••'}
              </span>
              <span className="text-xs text-gray-400">EFI</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] px-2 py-0.5 bg-white/10 text-gray-300 rounded-full font-medium">
                💜 Efi Monede
              </span>
      ...
      <div className="px-5 mb-5">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="glass rounded-2xl p-3.5 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 mb-0.5">Sent</p>
      ...
      <div className="px-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-[11px] text-[#7c6af7] flex items-center gap-0.5 font-medium active:opacity-70 transition-opacity"
          >
